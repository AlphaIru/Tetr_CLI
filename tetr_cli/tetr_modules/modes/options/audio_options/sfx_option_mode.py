...
class ModeClass(SideMenuToggleClass):
    ...
    def increment_frame(self, stdscr: window, pressed_keys: Set[str]) -> None:
        ...
        self.display_toggle(stdscr, "SFX Volume")
        if (
            ...
            if self.get_user_keybind("menu_back", menu_mode=True) & pressed_keys:
                set_setting("sfx_volume", str(self.old_value))
                return
            set_setting("sfx_volume", str(self.current_value))


if __name__ == "__main__":
    print("This is a sfx_option module, please run starter.py.")
