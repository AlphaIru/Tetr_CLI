"""This is a module for the wait room mode in multiplayer."""

# coding: utf-8

# from random import randint
from typing import List, Set

from curses import window, A_BOLD, A_REVERSE

from tetr_cli.tetr_modules.menu_core.base_mode import BaseModeClass

from tetr_cli.tetr_modules.modules.constants import MAX_NAME_LENGTH, NUMBER_SET, VALID_CHARS
from tetr_cli.tetr_modules.modules.database import set_temp
from tetr_cli.tetr_modules.modules.safe_curses import (
    calculate_centered_menu,
    safe_addstr,
)


class ModeClass(BaseModeClass):
    """This is the wait room mode class."""

    def __init__(self) -> None:
        """This will initialize this class."""
        super().__init__()
        self.__selected_option: int = 0
        self.__key_cooldown: int = 0
        # note selected_option:
        # 0: ip_1
        # 1: ip_2
        # 2: ip_3
        # 3: ip_4
        # 4: port
        # 5: username

        self.__ip: List[str] = ["0", "0", "0", "0"]
        self.__port: str = "0"
        self.__username: str = "Noob_Player2"  # + str(randint(2, 9999))

    def menu_control(self, pressed_keys: Set) -> None:
        """This will handle the menu controls."""
        if self.__key_cooldown > 0:
            self.__key_cooldown -= 1
        elif "up" in pressed_keys:
            if self.__selected_option > 4:
                self.__selected_option = 4
            else:
                self.__selected_option = 0
            self.__key_cooldown = 3
            self.sound_action["SFX"].append("select_move")
        elif "down" in pressed_keys:
            if self.__selected_option < 4:
                self.__selected_option = 4
            else:
                self.__selected_option = 5
            self.__key_cooldown = 3
            self.sound_action["SFX"].append("select_move")
        elif "left" in pressed_keys:
            self.__selected_option = max(0, self.__selected_option - 1)
            self.__key_cooldown = 3
            self.sound_action["SFX"].append("select_move")
        elif "right" in pressed_keys:
            self.__selected_option = min(4, self.__selected_option + 1)
            self.__key_cooldown = 3
            self.sound_action["SFX"].append("select_move")
        elif "enter" in pressed_keys:
            self.action["transition"] = ["Join_Room"]  # Placeholder
            self.action["ip"] = [".".join(self.__ip)]
            self.action["port"] = [self.__port]
            set_temp("username", self.__username)
            self.sound_action["SFX"].append("select_confirm")
        elif "esc" in pressed_keys:
            self.action["transition"] = ["Multi_Menu"]
            self.sound_action["SFX"].append("select_back")

    def display_menu(self, stdscr: window) -> None:
        """This will display the wait room menu."""
        start_y, start_x, _ = calculate_centered_menu(stdscr, [""])

        title: str = "Multiplayer Wait Room"
        safe_addstr(stdscr, start_y - 2, start_x - len(title) // 2, title, A_BOLD)

        safe_addstr(
            stdscr,
            start_y,
            start_x - 12,
            "IP:",
            A_REVERSE if self.__selected_option < 4 else 0,
        )
        for index in range(4):
            safe_addstr(
                stdscr,
                start_y,
                start_x - 8 + index * 4,
                f"{int(self.__ip[index]):>3d}",
                A_REVERSE if self.__selected_option == index else 0,
            )
            if index < 3:
                safe_addstr(
                    stdscr,
                    start_y,
                    start_x - 5 + index * 4,
                    ".",
                )

        safe_addstr(
            stdscr,
            start_y + 2,
            start_x - 12,
            "Port:",
            A_REVERSE if self.__selected_option == 4 else 0,
        )

        safe_addstr(
            stdscr,
            start_y + 2,
            start_x - 4,
            f"{int(self.__port):>5d}",
            A_REVERSE if self.__selected_option == 4 else 0,
        )

        safe_addstr(
            stdscr,
            start_y + 4,
            start_x - 12,
            "Username:",
            A_REVERSE if self.__selected_option == 5 else 0,
        )
        safe_addstr(
            stdscr,
            start_y + 4,
            start_x - 2,
            self.__username,
            A_REVERSE if self.__selected_option == 5 else 0,
        )

        statement: str = "Enter the peer IP and port to connect."
        safe_addstr(
            stdscr,
            start_y + 6,
            start_x - len(statement) // 2,
            statement,
        )

        statement = "Press Enter to Connect or Backspace to Return"
        safe_addstr(
            stdscr,
            start_y + 7,
            start_x - len(statement) // 2,
            statement,
        )

    def increment_frame(self, stdscr: window, pressed_keys: Set[str]) -> None:
        """This will progress the wait room based on the inputs."""
        self.menu_control(pressed_keys)
        self.display_menu(stdscr)

        if self.__key_cooldown > 0:
            return

        if "backspace" in pressed_keys:
            self.__key_cooldown = 3
            self.sound_action["SFX"].append("select_move")
            self.action["clear"] = []
            if self.__selected_option == 4:
                if len(self.__port) > 1:
                    self.__port = self.__port[:-1]
                    return
                self.__port = "0"
                return
            elif self.__selected_option <= 4:
                if len(self.__ip[self.__selected_option]) > 1:
                    self.__ip[self.__selected_option] = self.__ip[self.__selected_option][
                        :-1
                    ]
                    return
                self.__ip[self.__selected_option] = "0"
            elif len(self.__username) > 0:
                self.__username = self.__username[:-1]
            return

        if not pressed_keys:
            return

        self.__key_cooldown = 3
        input_key: str = pressed_keys.pop()

        if self.__selected_option <= 4:
            number_pressed: str = input_key if input_key in NUMBER_SET else ""
            if number_pressed == "":
                return
            self.action["clear"] = []
            self.sound_action["SFX"].append("select_move")
            if self.__selected_option < 4:
                self.__ip[self.__selected_option] = (
                    self.__ip[self.__selected_option] + number_pressed
                )
                if self.__ip[self.__selected_option].startswith("0"):
                    self.__ip[self.__selected_option] = self.__ip[
                        self.__selected_option
                    ][1:]
                if int(self.__ip[self.__selected_option]) > 255:
                    self.__ip[self.__selected_option] = "255"
                return
            self.__port = self.__port + number_pressed
            if self.__port.startswith("0"):
                self.__port = self.__port[1:]
            if int(self.__port) > 65535:
                self.__port = "65535"
            return

        if len(self.__username) >= MAX_NAME_LENGTH:
            return
        char_pressed: str = input_key if input_key in VALID_CHARS else ""
        if char_pressed == "":
            return

        self.action["clear"] = []
        self.sound_action["SFX"].append("select_move")
        self.__username = self.__username + char_pressed


if __name__ == "__main__":
    print("This is the join_room_mode module.")
